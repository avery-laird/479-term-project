...
let read_java_method_decl = (
  fun p lb ->
    ...
    let field_source_file = ref (Obj.magic (Sys.opaque_identity 0.0)) in
    ...
    with Yojson.End_of_object -> (
        if !bits0 <> 0xf then Atdgen_runtime.Oj_run.missing_fields p [| !bits0 |] [| "method_name"; "source_file"; "start_line"; "end_line" |];
        (
          {
